...
@mock.patch('provider.auth.core.auth_plugin', return_value=VALID_PLUGIN_NAME)
def test_auth_core_init_valid(mock_get):
    init()


@mock.patch(
    'provider.auth.core.auth_plugin', return_value=INCOMPLETE_PLUGIN_NAME
)
...
@mock.patch('provider.auth.core.auth_plugin', return_value=NO_PLUGIN_NAME)
